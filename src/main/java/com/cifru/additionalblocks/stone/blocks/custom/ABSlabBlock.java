 ...
 */
public class ABSlabBlock extends BaseBlock implements IWaterLoggable {
    ...
    public boolean canBeReplaced(BlockState state, BlockItemUseContext context){
        ...
        if(slabType != SlabType.DOUBLE && context.getItemInHand().getItem() == this.asItem()){
    ...
    @Override
    public boolean placeLiquid(IWorld level, BlockPos pos, BlockState state, FluidState fluid){
        return state.getValue(ABBlockProperties.SLAB_TYPE) != SlabType.DOUBLE && IWaterLoggable.super.placeLiquid(level, pos, state, fluid);
    }

    @Override
    public boolean canPlaceLiquid(IBlockReader level, BlockPos pos, BlockState state, Fluid fluid){
        return state.getValue(ABBlockProperties.SLAB_TYPE) != SlabType.DOUBLE && IWaterLoggable.super.canPlaceLiquid(level, pos, state, fluid);
    }

    @Override
    public boolean isPathfindable(BlockState state, IBlockReader level, BlockPos pos, PathType pathType){
        return pathType == PathType.WATER && state.getValue(ABBlockProperties.WATERLOGGED);
    }
